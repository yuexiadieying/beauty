...
var orderPayInit = function(){
    ...
    var appid;//得到的签名
    $.ajax({
        url:"weixin/getConfig",// 跳转到 action
        async:true,
        type:'get',
    ...
    if(productType=='offlineProduct')
    {
        var userAddressInfo = {};
        var needPayOrderList = {};
        console.log(buyOrderAddressId);
        ...
        if(buyOrderAddressId!=undefined||buyOrderAddressId!=null)
        {
            ...
            $.ajax({
                url:"system/getUserAddressUsedInfoByAddressId",// 跳转到 action
                ...
                success:function(data) {
                    ...
                    {
                        //没有登录，先去登录页
                        window.location.href = "ionic#/login/" + "businessHome";
                    }
                    else
                    {
        ...
        {
            $.ajax({
                url:"system/userAddressUsedInfo",// 跳转到 action
                beforeSend: function(request) {
                    request.setRequestHeader("logintoken", loginttoken);
                },
                async:true,
        ...
        $.ajax({
            url:"transaction/getNeedPayOrderListToRedis",// 跳转到 action
            beforeSend: function(request) {
                request.setRequestHeader("logintoken", loginttoken);
            },
            async:true,
            ...
            success:function(data) {
                ...
                if(data.errorInfo=='0x00006')
                {
                    window.location.href = "ionic#/login/" + "businessOrderPay";
                }
                else
                {
                    var totalPrice = 0;
...
var confirmPay = function(){
    if(receiveOrderAddressId==""&&productType=='offlineProduct')
    {
        alert("请先选择收货地址");
    }
    else
    {
        if(productType=='trainingProduct')
        {
            $.ajax({
                url:"account/pay/"+productType,// 跳转到 action
                beforeSend: function(request) {
                    request.setRequestHeader("logintoken", loginttoken);
                },
                async:true,
                        ...
                        wx.chooseWXPay({
                            ...
                            success: function (res) {
                                if(res.errMsg == "chooseWXPay:ok" ) {
                                    ...
                                    if(productType=='offlineProduct')
                                    {
                                        window.location.href = "ionic#/paySuccess";
                                    }else if(productType=='trainingProduct')
                                    {
                                        window.location.href = "ionic#/trainingProductList";
                                    }
                                }else{
                                    alert("支付失败,请重新支付")
                                }
        ...
        else if(productType=='offlineProduct')
        {
            //如果是微商城商品，则给订单补充上地址ID
            $.ajax({
                url:"transaction/updateBusinessOrderAddress",// 跳转到 action
                beforeSend: function(request) {
                    request.setRequestHeader("logintoken", loginttoken);
                },
                async:true,
                type:'get',
                data:{orderId:orderId,orderAddressId:receiveOrderAddressId},
                cache:false,
                success:function(data) {
                    if(data.result=="0x00001"){
                        $.ajax({
...
var check=function(){
   if($("#checkbox-id").prop("checked")){
       window.location.href="ionic#/invoice/"+orderId;
   }
}

var addressManagement = function(){
    window.location.href = "ionic#/addressManagement/orderPay.do";
}
