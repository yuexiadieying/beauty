    ...
    private class ProcessMessageThread extends Thread {
        ...
        public void run() {
            ...
            try
            {
                ...
                {
                    CarSocketService.getCarSocketService().processDeviceOnlineService(msgMap,channel);
                }
                else if("carAttack".equals(messageType))
                {
                    CarSocketService.getCarSocketService().processCarAttackService(msgMap,channel);
                }
                else if("carGoal".equals(messageType))
                {
                    CarSocketService.getCarSocketService().processCarGoalService(msgMap,channel);
                }
                else if("deviceProblem".equals(messageType))
                {
                    CarSocketService.getCarSocketService().processDeviceProblemService(msgMap,channel);
                }
                else if("gameServerHeart-Ctrl".equals(messageType))
                {
                    CarSocketService.getCarSocketService().processServerHeartBeatCtrl(msgMap,channel);
                }
