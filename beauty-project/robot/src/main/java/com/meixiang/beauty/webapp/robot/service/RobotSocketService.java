...
public class RobotSocketService {
    ...
    private static RobotSocketService robotSocketService = new RobotSocketService();

    public static RobotSocketService getRobotSocketService() {
        return robotSocketService;
    }

    private RobotSocketService(){
    }
    ...
    public final Map<String, String> robotDamageMapping = new ConcurrentHashMap<>();
    ...
    public final Map<String, WeixinUserBean> robotUserMapping = new ConcurrentHashMap<>();

    public final Map<String, String> userRobotMapping = new ConcurrentHashMap<>();
    ...
    public void processRobotAttackService(Map<String, Object> msgMap, Channel channel) {
        ...
        if("ongoing".equals(gameStatus))
        {
            ...
            if(damageLock==null||damageLock.equals("false"))
            {
                synchronized(deviceId)
                {
                    ...
                    String robotHP = robotDamageMapping.get(deviceId);
                    if(robotHP==null)
                    {
                        robotHP = gameParamDTO.getMaxHP();
                        robotHP = String.valueOf(Integer.parseInt(robotHP)-attackDamage);
                    }
                    else
                    {
                        robotHP = String.valueOf(Integer.parseInt(robotHP)-attackDamage);
                    ...
                    //进入晕厥状态
                    if(Integer.parseInt(robotHP)<=0)
                    {
                        damageLockMapping.put(deviceId,"true");

                        robotDamageMapping.put(deviceId,"0");
                        //死亡总数增加1
                        if(type.equals("red"))
                        {
                    ...
                    {
                        robotDamageMapping.put(deviceId,robotHP);
                    }
                }
            }
        }
    ...
    public void processRobotGoalService(Map<String, Object> msgMap, Channel channel) {
    ...
    public ResponseDTO startGame(String gamePlaceId) {
        ...
        if(gameStatus==null||"end".equals(gameStatus))
        {
            ...
            synchronized (gamePlaceId)
            {
                    ...
                    for(DeviceParamDTO deviceParamDTO:deviceParamDTOS)
                    {
                        ...
                        if(deviceOnlineStatus!=null)
                        {
                            if(deviceOnlineStatus.equals("online"))
                            {
                                ...
                                {
                                    doorOnlineNum++;
                                }else if(deviceParamDTO.getDeviceType().equals("robot"))
                                {
                                    robotOnlineNum++;
                                    if(robotUserMapping.get(deviceParamDTO.getDeviceId())!=null)
                                    {
                                        userBindNum++;
                                    }
                                }
                ...
                if(doorOnlineNum==2&&(robotOnlineNum%2==0)&&(robotOnlineNum!=0)&&(robotOnlineNum==userBindNum))
                {
                    String doorPart = "red";
                    String robotPart = "red";
                    ...
                    for(DeviceParamDTO deviceParamDTO:deviceParamDTOS)
                    {
                        ...
                        if(channel!=null)
                        {
                            ...
                            {
                                deviceParamDTO.setDeviceType(robotPart);
                                if(robotPart.equals("red"))
                                {
                                    robotPart = "blue";
                                    obj.put("part",robotPart);
                                    obj.put("deviceType","robot");
                                    devicePartMapping.put(deviceParamDTO.getDeviceId(),robotPart);
                                }
                                else if(robotPart.equals("blue"))
                                {
                                    robotPart = "red";
                                    obj.put("part",robotPart);
                                    obj.put("deviceType","robot");
    ...
    public ResponseDTO getRobotDamage(String deviceId) {
        ResponseDTO responseDTO = new ResponseDTO();

        responseDTO.setResponseData(robotDamageMapping.get(deviceId));

        responseDTO.setResult(StatusConstant.SUCCESS);
        return responseDTO;
    }

    public void bindUserAndRobot(String robotId, WeixinUserBean weixinUserBean) {
        robotUserMapping.put(robotId,weixinUserBean);
        userRobotMapping.put(weixinUserBean.getOpenid(),robotId);
    }
    ...
    public class GameStopThread extends Thread {
        ...
        public void run() {
            ...
            for(DeviceParamDTO deviceParamDTO:deviceParamDTOS)
            {
                ...
                removeCurrentHashMap(robotDamageMapping,deviceParamDTO.getDeviceId());
                removeCurrentHashMap(damageLockMapping,deviceParamDTO.getDeviceId());
                //removeCurrentHashMap(devicePartMapping,deviceParamDTO.getDeviceId());
                removeCurrentHashMap(robotDamageMapping,deviceParamDTO.getDeviceId());
                removeCurrentHashMap(robotUserMapping,deviceParamDTO.getDeviceId());
                removeCurrentValueHashMap(userRobotMapping,deviceParamDTO.getDeviceId());
            }
...
//                    }
                    ...
                    robotDamageMapping.put(deviceParamDTO.getDeviceId(),gameParamDTO.getMaxHP());
    ...
    public void processServerHeartBeatCtrl(Map<String, Object> msgMap, Channel channel) {
        ...
        Query query = new Query(Criteria.where("gamePlaceId").is(gamePlaceId));
        List<DeviceParamDTO> deviceParamDTOS = mongoTemplate.find(query,DeviceParamDTO.class,"deviceParam");
        List<DeviceParamDTO> bluePartRobotList = new ArrayList<>();
        List<DeviceParamDTO> redPartRobotList = new ArrayList<>();
        HashMap<String,String> doorMap = new HashMap<>();
        List<HashMap<String,String>> robotDamageList = new ArrayList<>();
        List<HashMap<String,String>> robotDeadList = new ArrayList<>();
        List<HashMap<String,Object>> robotUserList = new ArrayList<>();
        for(DeviceParamDTO deviceParamDTO:deviceParamDTOS)
        {
            if("blue".equals(devicePartMapping.get(deviceParamDTO.getDeviceId())))
            {
                ...
                {
                    bluePartRobotList.add(deviceParamDTO);
                }
            }
            else if("red".equals(devicePartMapping.get(deviceParamDTO.getDeviceId())))
            {
                ...
                {
                    redPartRobotList.add(deviceParamDTO);
                }
            }

            HashMap<String,String> robotDamageMap = new HashMap<>();
            if(robotDamageMapping.get(deviceParamDTO.getDeviceId())==null)
            {
                robotDamageMap.put("deviceId",deviceParamDTO.getDeviceId());
                robotDamageMap.put("value","100");
            }
            else
            {
                robotDamageMap.put("deviceId",deviceParamDTO.getDeviceId());
                robotDamageMap.put("value",robotDamageMapping.get(deviceParamDTO.getDeviceId()));
            }
            robotDamageList.add(robotDamageMap);


            HashMap<String,String> robotDeadMap = new HashMap<>();
            if(deviceDeadMapping.get(deviceParamDTO.getDeviceId())==null)
            {
                robotDeadMap.put("deviceId",deviceParamDTO.getDeviceId());
                robotDeadMap.put("value","0");
            }
            else
            {
                robotDeadMap.put("deviceId",deviceParamDTO.getDeviceId());
                robotDeadMap.put("value",String.valueOf(deviceDeadMapping.get(deviceParamDTO.getDeviceId())));
            }
            robotDeadList.add(robotDeadMap);

            HashMap<String,Object> robotUserMap = new HashMap<>();
            if(robotUserMapping.get(deviceParamDTO.getDeviceId())!=null)
            {
                robotUserMap.put("robotId",deviceParamDTO.getDeviceId());
                robotUserMap.put("value",robotUserMapping.get(deviceParamDTO.getDeviceId()));
                robotUserList.add(robotUserMap);
            }
        }

        JSONObject obj = new JSONObject();
        obj.put("bluePartRobotList", bluePartRobotList);
        obj.put("redPartRobotList", redPartRobotList);
        obj.put("doorMap", doorMap);
        obj.put("robotDeadList",robotDeadList);
        obj.put("robotDamageList", robotDamageList);
        obj.put("robotUserList",robotUserList);
